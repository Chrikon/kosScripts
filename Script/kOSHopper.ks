...
parameter altitude_d is 2*ship:orbit:body:atm:height,thrust_d is 1,direction_d is 90.

function acheiveTWR{
    parameter var,engi,min_t.
    ...
    if factor>=1
        ...
        return 0.
    else if factor<=min_t
        return min_t.
    else
        return factor.
}

function findMinThrottle{
    parameter engi.
    if engi:empty
        return 0.
    else{
        ...
        for i in engi{
            if i:minthrottle>min_t
                set min_t to i:minthrottle.
        }   
        return min_t.
    }
}
...
function calculateExhaustSpeedg{
    ...
    for i in ind{
        if(0.00986923267*ship:sensors:pres>0.0001)
...
declare local vsteer is 1.
lock throttle to vsteer.
stage.
declare local engi is findActiveEngine().
declare local min_t is findMinThrottle(engi).
if (ship_throttle<min_t)
    set vsteer to min_t.
wait 2.71.
stage.
until ship:apoapsis>ship_apoapsis{
    if ship:verticalSpeed>=100 and ship:altitude<5000{
        set pi to 70.
        print "Heading :"+di+",70".
    }else if ship:altitude>=5000{
        set pi to round(90*(1-(ship:apoapsis/altitude_d)^0.34),2).
        print "Heading :90,"+pi+","+eta:apoapsis.
        if ship:altitude>=2*ship:orbit:body:atm:height/3 and flag{
            set flag to false.
            set vsteer to 1.
          //  print flag.
        }
    ...
    if  ship:maxthrust=0{
       ...
       set min_t to findMinThrottle(engi).
       if min_t=1
        set flag to false.
       set RCS to TRUE.
       wait 2.63.
    }else if ship:maxThrust<thrust{
       print "Booster's cut off".
       // lock throttle to 1.
        wait until stage:ready.
        stage.        
    }
    if(flag){
        ...
        set vsteer to acheiveTWR(vsteer,engi,min_t).
    }
}
lock throttle to 0.
unlock steering.
