...
function acheiveTWR{
    ...
    local factor is var+(1/ship:sensors:acc:mag)*ratio.
...
function checkBosterStatus{
   LIST ENGINES in eng.
    for engi in eng{
        if (engi:flameout){
            print "Booster flameout purge".
...
declare local boosterOn is true.
...
until ship:apoapsis>ship_apoapsis{
    ...
    }else if boosterOn and checkBosterStatus(){
       wait 0.1.
       set boosterOn to false.
       set engi to findActiveEngine(). 
    }
    if(flag){
...
function simpleExecution{
    ...
    local a is x:time-(burnTime/2)-t_spool-1.
