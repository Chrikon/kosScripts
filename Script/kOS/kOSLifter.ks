...
parameter altitude_d is 100000,thrust_d is 0.2.
...
function calculateExhaustSpeedg{
    ...
    for i in ind{
        if(0.00980966720709589*ship:sensors:pres>0.001)
            set paranomastis to paranomastis+i:possiblethrustat(0.00980966720709589*ship:sensors:pres)/i:ispat(0.00980966720709589*ship:sensors:pres).
        else
...
until ship:apoapsis>ship_altitude{
    if ship:verticalSpeed>=100 and ship:altitude<7000{
        ...
        if ship:altitude>=50000 and flag{
            set flag to false.
            set vsteer to 1.
          //  print flag.
        }
...
function main{
    if(0.00980966720709589*ship:sensors:pres>0.1){
    set ag1 to true.
